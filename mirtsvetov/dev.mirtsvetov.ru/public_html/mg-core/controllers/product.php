...
class Controllers_Product extends BaseController {
  ...
  function __construct() {
    ...
    if(USER::access('wholesales') > 0) {
      ...
      $res = DB::query('SELECT DISTINCT count, price FROM '.PREFIX.'wholesales_sys WHERE product_id = '.DB::quoteInt($product['id']).' 
        AND (variant_id = '.DB::quoteInt($product['variant']).' OR variant_id = 0) AND `group` = '.User::access('wholesales').' ORDER BY count ASC');
