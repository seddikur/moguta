...
elFinder.prototype._options = {
	...
	contextmenu : {
		...
		cwd    : ['undo', 'redo', '|', 'back', 'up', 'reload', '|', 'upload', 'mkdir', 'mkfile', 'paste', '|', 'empty', '|', 'view', 'sort', 'selectall', 'colwidth', '|', 'info', '|', 'fullscreen'],
				...
				.on('posinit', function() {
					...
					if (! css && ! dialog.data('resizing')) {
						...
						if (fm.options.dialogContained || nodeFull) {
							...
							minTop =  Math.max(nodeOffset.top * -1 + winSize.scrTop, 0);
