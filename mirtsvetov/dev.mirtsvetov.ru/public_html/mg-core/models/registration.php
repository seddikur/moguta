    ...
    if ('full' == $mode || 'nopass' == $mode ){
      ...
      if (!MG::checkEmail($userDataEmail)) {
          $msg = 'msg__reg_wrong_email';
            // Елси email не корректный, то проверяем номер телефона.
            preg_match('/((^\+?\d)?[\d\-\ ]?((?:\()\d*(?:\)))|((?|([\-\ ]{1}[\d]{1,})|(\+?[\d]*))*)(\d$))/', $userDataPhone, $lenPhone);
            if(strlen($lenPhone[0]) != strlen($userDataPhone)){
              $msg = 'msg__reg_wrong_login';
              // $error .= '<span class="errorEmail">Неверно заполнен email или номер телефона</span>';
