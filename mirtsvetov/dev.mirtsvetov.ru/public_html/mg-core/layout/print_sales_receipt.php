...
<div class="form-wrapper">
  ...
  if ($data['order']['user_comment']) {
    echo '<strong>Комментарий пользователя: </strong>'.MG::replaceBBcodes($data['order']['user_comment']).'<br />';
  }
  if ($data['order']['comment']) {
    echo '<strong>Комментарий менеджера: </strong>'.MG::replaceBBcodes($data['order']['comment']).'<br />';
  }
   ...
   // viewData($data);
    foreach ($data['perOrders'] as $perOrder) {
      ...
      <tr>
        ...
        <td ><?php echo $perOrder['count'].' '. (empty($perOrder['unit']) ? 'шт.' : $perOrder['unit'])?></td>
...
<?php 
  ...
  if($propertyOrder['showDeliveryCost'] == 'true' && $data['order']['delivery_id'] != 3){
?>
      <tr>
        <td style="padding: 5px;"><?php echo $i++;?></td>
        <td ><?php echo '' ?></td>
        <td cellpadding="5">
        <?php echo htmlspecialchars_decode(str_replace('&amp;', '&', "Доставка товара"));?>
        </td>        
        <td ><?php echo '1 '. (empty($perOrder['unit']) ? 'шт.' : $perOrder['unit'])?></td>
        <td ><?php echo MG::numberFormat($data['order']['delivery_cost']).' '.$data['currency'];?></td>                
        <td ><?php echo MG::numberFormat($data['order']['delivery_cost']).' '.$data['currency'];?></td>
      </tr>  

  
  <?php $totalPrice = $totalPrice + $data['order']['delivery_cost']; ?>
  <?php }?>   
  </table>
