...
class URL {
  ...
  private function __construct() {
    // установка языка для мультиязычности
    MG::setLang();
  ...
  public static function multiLangLink($data = '') {
    // Было бы здорово перелопатить этот кусок кода,потому что не пронятно зачем делать замену отдельно для форм и отдельно для тегов <a>
    if(LANG != 'LANG' && LANG != 'default') {
      preg_match_all('/<a[^>]*href=\"?([^>]*>)/', $data, $matches);
      unset($matches[0]);
      $newLink = array();
      foreach ($matches[1] as &$item) {
        if((substr_count($item, SITE) == 1)&&(substr_count($item, '.png') != 1)&&(substr_count($item, '.jpg') != 1)&&(substr_count($item, SITE.'/'.LANG) === 0)) {
      ...
      $newLink = array();
      foreach ($matches[1] as &$item) {
        if((substr_count($item, SITE) === 1)&&(substr_count($item, SITE.'/'.LANG) === 0)) {
          $newLink[$item] = str_replace(SITE, SITE.'/'.LANG, $item);
        }
        if(substr_count($item, '/'.LANG) === 0 && (substr_count($item, SITE) === 0)) {
          if(substr_count($item, 'http') > 0 || substr_count($item, 'https')>0 ){
            continue;
          }
          $site = explode('/', SITE.$item);
          $site = implode('/', array_unique($site));
          $newLink[$item] = str_replace(SITE, SITE.'/'.LANG, $site);
          if (strpos($_SERVER['REDIRECT_URL'], '/en/') !== false) {
            $dataSite = explode('/'.LANG.'/', $_SERVER['REDIRECT_URL']);
            $dataSite = '/'.$dataSite[1];
            if (strpos($newLink[$item], SITE.'/'.LANG.'?') !== false && strpos($newLink[$item], 'apply-filter-form') !== false) {
              $newLink[$item] = str_replace('?', $dataSite.'?', $newLink[$item]);
            }
          }
          // $newLink[$item] = str_replace(SITE, SITE.'/'.LANG, $item);
        }
