...
class Backup {
	...
	static function checkSystem() {// вывод информации о сервере
		...
		if(MG::getSetting('useDefaultSettings') == 'false'){			
			...
			$html .= '<span class="question__wrap" flow="left" tooltip="'.$lang['RESET_SETTINGS_TOOLTIP'].'">
			...
			$html .= '<span class="question__wrap" flow="left" tooltip="'.$lang['CUSTOM_SETTINGS_TOOLTIP'].'">
						<i class="fa fa-question-circle" aria-hidden="true"></i>
					</span>';
		}
	...
	static function unpackQuickstartTables() {
		...
		while (count($tables) > 0) {
			...
			if ($table == 'setting') {
				...
				DB::query('ALTER TABLE `'.PREFIX.'setting` AUTO_INCREMENT = 1');
				DB::query('COMMIT');
			}
