...
class Pactioner extends Actioner {
  ...
  public function getEntity() {
    ...
    if ($row = DB::fetchAssoc($res)) {
      // В ссылке на изображение меняем плейсхолдер на домен сайта
      if (strpos($row['icon'], '#SITE#') !== 0) {
        $row['icon'] = str_replace('#SITE#', SITE, $row['icon']);
      }
      $this->data = $row;
      return true;
    }
  ...
  public function saveEntity() {
    ...
    // Для иконки типа "изображение по ссылки" меняем абсолютный путь до изображения
    // на путь со специальным плейсхолдером, который на выводе будет снова заменён на домен сайта
    // Это нужно для того, чтобы изображения не исчезали при смене домена
    if (strpos($_POST['icon'], 'src="'.SITE) !== false) {
      $_POST['icon'] = str_replace('src="'.SITE, 'src="#SITE#', $_POST['icon']);
    }
  ...
  public function getTrigger() {
    ...
    if ($row = DB::fetchAssoc($res)) {
      ...
      while ($el = DB::fetchAssoc($elements)) {
        // плейсхолдер в ссылке на изображение меняем на домен сайта
        if (strpos($el['icon'], '#SITE#') !== 0) {
          $el['icon'] = str_replace('#SITE#', SITE, $el['icon']);
        }
        $row['elements'][] = $el;
      }
