    ...
    if ($arg['id']) {
      ...
      if ($trigger = DB::fetchArray($rez)) {
        ...
        while ($elements = DB::fetchArray($res)) {
          // для ссылок на изображение плейсхолдер меняем обратно на домен сайта
          if (strpos($elements['icon'], '#SITE#') !== 0) {
            $elements['icon'] = str_replace('#SITE#', SITE, $elements['icon']);
          }
