...
$(document).ready(function() {
    ...
    $('body').on('click', '.change-payment', function () {
        ...
        $.ajax({
            ...
            cache: false,
            success: function(response) {
                const cform = select.closest('.c-form');
                if (response.error) {
                    cform.children().hide();
                    if (cform.find('.alert-change-payment').length == 0) {
                        const alert = locale.noChangePayment ? locale.noChangePayment : 'Нельзя изменить способ оплаты'
                        cform.append('<div class="alert-change-payment">' + alert + '</div>');
                    } else {
                        cform.find('.alert-change-payment').show();
                    }
                    return;
                } else {
                    cform.children().show();
                    cform.find('.alert-change-payment').hide();
                }
