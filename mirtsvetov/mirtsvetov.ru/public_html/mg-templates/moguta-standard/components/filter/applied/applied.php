...
if (empty($data) && empty($_GET['sale']) && empty($_GET['new']) && empty($_GET['recommend'])) {
...
<div class="l-col min-0--12 c-apply__wrapper">
    <div class="c-apply apply-filter-line">
        ...
        <form action="<?php echo SITE.URL::getClearUri(); ?>" class="c-apply__form apply-filter-form"
              data-print-res="<?php echo MG::getSetting('printFilterResult') ?>" <?php echo $style ?>>
            <ul class="c-apply__tags filter-tags">
                <?php
                    $specOffers = [
                        'sale' => lang('filterSaleS'),
                        'new' => lang('filterNewS'),
                        'recommend' => lang('filterRecommendS'),
                    ];
                    $applyedSpecOffers = [];
                    foreach ($specOffers as $specOffer => $specOfferName) {
                        if (!empty($_GET[$specOffer])) {
                            $applyedSpecOffers[$specOffer] = $specOfferName;
                        }
                    }
                    if ($applyedSpecOffers) :
                ?>
                    <li class="c-apply__tags--item apply-filter-item">
                        ...
                        <ul class="c-apply__tags--values filter-values">
                            <?php foreach ($applyedSpecOffers as $specOffer => $specOfferName) {
                                ?>
                                <li class="c-apply__tags--value apply-filter-item-value">
                                    <?php echo $specOfferName; ?>
