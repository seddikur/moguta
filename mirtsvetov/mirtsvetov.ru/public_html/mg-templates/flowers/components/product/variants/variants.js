...
function changeMainImgToVariant(response, page) {
    ...
    var thumbs_slider = $('.js-thumbs-slider');
    
    var main_first =  main_slider.find('[data-index=0]');
    var thumbs_first = thumbs_slider.find('[data-index=0]');

    if (response.data.image_orig !== '' && !response.data.image_orig.includes('no-img.jpg')) {
        main_first.parent().data('src', response.data.image_orig);
        main_first.find('.js-zoom-img').attr('style', 'background-image: url(' + response.data.image_orig + ')');
        if (thumbs_slider.length) {
            thumbs_first.click();
            changeImgSrc(thumbs_first, '30', response.data.image_thumbs);
            changeImgSrc(main_first, '70', response.data.image_thumbs);
        } else {
            changeImgSrc(main_first, '70', response.data.image_thumbs);
        }
    }
...
function changeImgSrc(imgElem, size, thumbsArr) {
    imgElem.attr('src', thumbsArr[size]) .attr('srcset', thumbsArr['2x' + size] + ' 2x');
}
