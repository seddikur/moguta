     ...
     <?php /* if(!USER::getUserInfoByEmail($data['email'])->activity) { ?>
      ...
      <?php
        if (
          ...
          ) && (!isset($data['showPaymentForm']) || $data['showPaymentsForm'])
        ):
      ?>
            ...
            <?php if (!empty($data['productPositions']) || $data['adminOrder']) : ?>
              ...
              <?php foreach ($data['productPositions'] as $product): ?>
                ...
                <?php 
                  ...
                  $intProduct = getProduct($product['id'], $product['variant_id']);
                  if ($intProduct) {
                    $vat += (float) $product['count'] * (float) $intProduct['opf_21'];
                  }
                ?>
                ...
                <tr>
                  ...
                  <?php if ($intProduct): ?>
                    <td style="font-size: 13px; padding: 5px 9px;" align="center">
                      <?php echo $product['count'] .' × '. $intProduct['opf_21'] .' '. $product['unit']; ?>
                    </td>
                  <?php else: ?>
                    <td style="font-size: 13px; padding: 5px 9px;" align="center">
                      <?php echo $product['count'] ?> <?php echo $product['unit']; ?>
                    </td>
                  <?php endif; ?>
        ...
        if ($data['deliveryCost'] > 0) { ?>
          <tr>
            ...
            <td align="right" style="padding:5px 9px 0 9px;font-size: 13px;font-weight:bold;">
              <span><?php echo  MG::numberFormat($data['result']).' '.$data['currency'] ?></span>
            </td>
          </tr>
          <tr>
            ...
            <td align="right" style="padding:2px 9px;font-size: 13px;font-weight:bold;">
              <span><?php echo  MG::numberFormat($data['deliveryCost']).' '.$data['currency'] ?></span>
            </td>
          </tr>
        <?php }?>
          ...
          <tr>
            ...
            <td align="right" style="padding: 2px 9px 5px 9px; color: #333; text-align: right; font-size: 13px; font-weight: bold;">
                <strong>
                  <span><?php echo $count .' / '. $vat .' шт.';?></span>
                </strong>
            </td>
          </tr>
