<?php
	...
	$site = '//'.$_SERVER['SERVER_NAME'].$siteFolder;
...
<head>
	...
	<link href="<?php echo $site?>/mg-admin/design/css/style.css?<?php echo filemtime(ADMIN_DIR.'/design/css/style.css') ?>" rel="stylesheet">
	<!--  -->
			    ...
			    success: function (response) {
			        ...
			        if(response.invalid) {
			        	...
			        	if(response.error) {
			        		...
			        		if(response.days > 0) {
                                $('.key-info').html('Чтобы восстановить работу сайта, необходимо установить на вашем веб-сервере версию PHP ' + response.lastPHP + '<br>Так же вы можете обновить CMS до актуальной версии.<br><br>');
			        			$('.key, .enter-key-btn').hide();
			        		} else {
								if (typeof(response.message) !== 'undefined' && response.message.search(/Ваша версия PHP [\d]+\.[\d]+ не поддерживается!/) === 0) {
									$('.key-info').html('<b>Чтобы восстановить работу сайта, необходимо установить на вашем веб-сервере версию PHP ' + response.lastPHP + '</b><br>' + response.message);
									$('.key, .enter-key-btn, .update-btn').hide();
								} else {
									$('.key-info').html('<b>Чтобы восстановить работу сайта, необходимо установить на вашем веб-сервере версию PHP ' + response.lastPHP + '</b><br>Так же вы можете обновить CMS до актуальной версии.<br><br><div class="error-block">Срок действия ключа истек, обновление недоступно.</div>');
									$('.key, .enter-key-btn, .update-btn').hide();
								}
			        		}
