	...
	return {
		...
		initEvents: function() {
			...
			$('.section-catalog').on('click', '.property-file-select', function() {
				...
				admin.openUploader(function(file) {userProperty.addFileToProp(file, propId);}, null, 'uploads', false, true);
			});

			$('.section-catalog').on('click', '.property-file-remove', function() {
				const propId = $(this).data('id');
				const containerId = $(this).closest('.property-file-name-container').attr('data-id');
				if (!propId) {
					admin.indication('error', 'Неизвестная ошибка');
					return false;
				}
				userProperty.removeFileFromProp(propId, containerId);
			});
			...
			$('.section-catalog').on('change', '.filter-container input[name="insideCat"]', function () {
				...
				var insideCatValue = $(this).prop('checked') ? 'true' : 'false';
				$('.section-catalog .filter-container input[name="insideCat"]').val(insideCatValue)
				
				var request = $("form[name=filter]").formSerialize();
				admin.show("catalog.php", cookie("type"), request + "&page=0&cat_id=" + cat_id + '&displayFilter=1', catalog.init);
			});
			...
			$('.section-catalog').on('click', '#add-product-wrapper .url-popup .apply-url', function () {
				...
				if ($(this).closest('.additional_uploads_container_variants').length !== 0) return;
				admin.ajaxRequest({
			...
			$('.section-catalog').on('change', 'select[name="operation"]', function () {
				...
				if ($(this).val() == 'getcsv') {
					$('select#csvEncoding').show();
				} else {
					$('select#csvEncoding').hide();
				}
			...
			$('.section-catalog').on('click', '.color-td', function(e) {
				const tds = $(this).closest('tr').find('.color-size-td');
				e.preventDefault();
				if (e.ctrlKey || e.metaKey) {
					tds.each((index, item) => {
						$(item).find('.checkbox input').prop('checked', !$(item).find('.checkbox input').is(":checked"));
					});
				} else {
					let isChecked = false;
					let counter = 0;
					tds.each((index, item) => {
						if ($(item).find('.checkbox input').is(":checked")) {
							counter++
						}
					});
					...
					tds.each((index, item) => {
						$(item).find('.checkbox input').prop('checked', isChecked);
					})
				}

				$('.size-none .checkbox input').prop('checked', false);
				$('.color-none .checkbox input').prop('checked', false);
			});
			$('.section-catalog').on('click', '.size-td', function(e) {
				e.preventDefault();
				const tr = $(this).closest('tr');
				const trs = tr.closest('table').find('tr');
				let pos = 0;
				...
				if (e.ctrlKey || e.metaKey) {
					trs.each((index, item) => {
						$(item).find('.color-size-td').each((index, item) => {
							if (index === pos - 1 ) {
								$(item).find('.checkbox input').prop('checked', !$(item).find('.checkbox input').is(":checked"));
							}
						})
			...
			$('.section-catalog').on('click', '.not-size', function(e) {
				e.preventDefault();
				$(this).closest('table').find('.color-size-td .checkbox input').prop('checked', false);
				$(this).closest('table').find('td.size-none .checkbox input').prop('checked', false);
				const sizeNone = $(this).closest('table').find('td.color-none');
				if (e.ctrlKey || e.metaKey) {
					sizeNone.each((index, item) => {
						$(item).find('.checkbox input').prop('checked', !$(item).find('.checkbox input').is(':checked'));
					});
				} else {
					let counter = 0;
			...
			$('.section-catalog').on('click', '.not-color', function(e) {
				e.preventDefault();
				$(this).closest('table').find('.color-size-td .checkbox input').prop('checked', false);
				$(this).closest('table').find('td.color-none .checkbox input').prop('checked', false);
				const sizeNone = $(this).closest('table').find('td.size-none');
				if (e.ctrlKey || e.metaKey) {
					sizeNone.each((index, item) => {
						$(item).find('.checkbox input').prop('checked', !$(item).find('.checkbox input').is(':checked'));
					});
					...
					if (!response.success) {
						...
						setTimeout(function () {
							if (confirm(lang.CATALOG_MESSAGE_1 + response.file + lang.CATALOG_MESSAGE_2)) {
								const fileLink = mgBaseDir + '/' + response.file + '?timestamp=' + Number(Date.now()).toString();
...
//            $('body').append('<iframe src="'+mgBaseDir+'/'+response.file+'" style="display: none;"></iframe>');
		...
		runOperation: function (operation, skipConfirm) {
			...
			if ($('select#csvEncoding').is(':visible')) {
				data.csv_encoding = $('select#csvEncoding').val();
				if (!data.csv_encoding) {
					data.csv_encoding = 'win1251';
				}
			}
							...
							if (response.data.fileyml) {
								...
								setTimeout(function () {
									if (confirm(lang.CATALOG_MESSAGE_1 + response.data.fileyml + lang.CATALOG_MESSAGE_2)) {
										location.href = mgBaseDir + '/mg-admin/?yml=1&filename=' + response.data.fileymlpath + '&timestamp=' + Number(Date.now()).toString();
									}
								}, 2000);
							}
							admin.refreshPanel();
		...
		getProductByFilter: function () {
			var request = $("form[name=filter]").formSerialize();
			admin.show("catalog.php", "adminpage", request + '&applyFilter=1&displayFilter=1', catalog.init);
			return false;
		},
