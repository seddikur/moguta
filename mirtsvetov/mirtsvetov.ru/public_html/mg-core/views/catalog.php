...
<?php if (empty($data['searchData'])): ?>
    ...
    <div class="l-row">
        <!-- c-title - start -->
        <div class="l-col min-0--12 c-title__categoty" itemtype="http://schema.org/AggregateOffer" itemscope="" itemprop="offers">
            <h1 class="c-title" itemprop="name"><?php echo $data['titeCategory'] ?></h1>
            <meta content="<?php echo count($data['items']) ?>" itemprop="offerCount"> <!-- количество товара -->
            <?php if (PHP_VERSION>= 5.5):?>
                <meta content="<?php echo max(array_column($data['items'], 'price_course')) ?>" itemprop="highPrice"><!-- Высшая цена -->
                <meta content="<?php echo min(array_column($data['items'], 'price_course')) ?>" itemprop="lowPrice"><!-- Низшая цена -->
            <?php endif; ?>
            <meta content="<?php echo MG::getSetting('currencyShopIso'); ?>" itemprop="priceCurrency"><!-- Валюта -->
        </div>
        <!-- c-title - end -->
