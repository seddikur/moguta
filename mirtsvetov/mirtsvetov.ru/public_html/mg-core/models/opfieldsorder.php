...
//				    viewData($item);
	...
	static function getFieldTitle($fieldName) {
		switch ($fieldName) {
			...
			case 'shop_yur_id': return lang('orderShopYur');
	...
	static function getOrderFormPublicArr() {
		...
		$defaultYur = MG::getSetting('propertyOrder', true);
		$addReqs = MG::getSetting('addRequisites',true);
		$fields['shop_yur_id']['vars'] = array(
			array('val' => 0, 'text' => $defaultYur['nameyur']),
		);	
		if(!empty($addReqs)){
			foreach($addReqs as $reqKey => $req){
				if(empty($req['nameyur'])) continue;
					array_push($fields['shop_yur_id']['vars'], array('val' => $reqKey, 'text' => $req['nameyur']));		
			}
		}
	...
	static function rebuildJs($fields, $addEnctype=false) {
		...
		foreach ($fields as $fieldName => $field) {
			...
			if (!isset($field['type']) || $field['type'] != 'set') {
				...
				if($fieldName === 'shop_yur_id'){
				$show = '				$(\'form[action*="/order?creation=1"] [name='.$fieldName.']\').closest(\'.js-orderFromItem\').show();'.PHP_EOL;
				$hide = '				$(\'form[action*="/order?creation=1"] [name='.$fieldName.']\').closest(\'.js-orderFromItem\').hide();'.PHP_EOL;
				}
			}
