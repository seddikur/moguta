...
<div class="mg-filter-head">
    ...
    $isActiveSortStyle = '';
    if(!empty($sortFields)){
        $isActiveSortStyle = 'style="display: none;"';
        foreach($sortFields as $key => $sortValue){
            if($sortValue['enable'] === 'true'){
                $isActiveSortStyle = '';
                break;
            }
        }
    ...
    <?php foreach ($data['property'] as $name => $prop) {
        switch ($prop['type']) {
            case 'select':
            {
                ...
                <div class="wrapper-field" <?php echo $isActiveSortStyle; ?>>
                    <?php if(!empty($sortFields) && empty($prop['option']['null'])): ?>
                        <div class="filter-select">
                            <div class="select"><span class="label-field"><?php echo $prop['label'] ?></span>
                                <select name="<?php echo $name ?>"
                                        class="last-items-dropdown">
                                    <?php if($prop['option'][$prop['selected']]['enable'] === 'false') : ?>
                                        <option value="" selected="selected" disabled>
                                            <?php echo $lang['NO_SORT_VARIANTS']; ?>
                                        </option>                                        
                                    <?php endif; ?>
                                <?php foreach ($prop['option'] as $key => $sortValue) {
                                    if($sortValue['enable'] === 'false') continue;
                                    $selected = ($prop['selected'] === $key . "") ? 'selected="selected"' : ''; ?>
                                    <option value="<?php echo $key ?>" <?php echo $selected ?>>
                                        <?php echo $lang[$sortValue['lang']]; ?>
                                    </option>
                                <?php } ?>
                            </select></div>
                        <?php if ($name == 'cat_id') {
                            ...
                            <div class="checkbox"><?php echo $lang['FILTR_PRICE7'] ?>
                                <input type="checkbox"
        ...
        case 'between': {
            if (isset($prop['special']) && $prop['special'] == 'date') { ?>
             ...
             <ul class="period-date">
               <li>
                ...
                <?php } else { ?>
                    <?php if(MG::getSetting('printPriceFilterBlock') !== 'false') { ?>
                        <div class="wrapper-field range-field">
                            <div class="price-slider-wrapper">
                                <ul class="price-slider-list">
                                    <li><label for="minCost" class="label-field from"><?php echo $prop['label1'] ?></label><input
                                                type="text" id="minCost"
                                                class="price-input start-<?php echo $prop['class'] ?>  price-input"
                                                data-fact-min="<?php echo $prop['factMin'] ?>" name="<?php echo $name ?>[]"
                                                value="<?php echo $prop['min'] ?>"/></li>
                                    <li><label for="maxCost" class="label-field to"><?php echo $prop['label2'] ?></label>
                                        <input type="text"
                                            id="maxCost"
                                            class="price-input end-<?php echo $prop['class'] ?>  price-input"
                                            data-fact-max="<?php echo $prop['factMax'] ?>"
                                            name="<?php echo $name ?>[]"
                                            value="<?php echo $prop['max'] ?>"/><span><?php echo MG::getSetting('currency') ?></span>
                                    </li>
                                </ul>
                                <div class="clear"></div>
            ...
            case 'hidden':
            { ?>
                <input type="hidden" name="<?php echo $name ?>" value="<?php echo $prop['value'] ?>"
                       class="price-input"/>
                <?php break;
            }
            ...
            case 'text':
            {
                if (!empty($prop['special'])) { ?>
                    <div class="wrapper-field"><span class="label-field"><?php echo $prop['label'] ?>:</span> <input
                                type="text" name="<?php echo $name ?>[]" value="<?php echo $prop['value'] ?>"
                    ...
                    <li>
                        <label style="margin-bottom:8px;">
                            <input type="checkbox" name="sale" value="1"
                                   class="mg-filter-prop-checkbox" <?php echo isset($_REQUEST['sale']) ? 'checked' : ''; ?>>
                    ...
                    <li>
                        <label style="margin-bottom:8px;">
                            <input type="checkbox" name="new" value="1"
                                   class="mg-filter-prop-checkbox" <?php echo isset($_REQUEST['new']) ? 'checked' : ''; ?>>
                    ...
                    <li>
                        <label style="margin-bottom:8px;">
                            <input type="checkbox" name="recommend" value="1"
                                   class="mg-filter-prop-checkbox" <?php echo isset($_REQUEST['recommend']) ? 'checked' : ''; ?>>
...
<?php if (MG::get('controller') == 'controllers_catalog' || (isset($_REQUEST['mguniqueurl']) && $_REQUEST['mguniqueurl'] == 'catalog.php')) { ?>
    <div class="mg-filter-body">
        <?php echo $data['propertyFilter']; ?>
    </div>
<?php } ?>
<?php if (MG::get('controller') == 'controllers_users' || (isset($_REQUEST['mguniqueurl']) && $_REQUEST['mguniqueurl'] == 'users.php')) { ?>
    <div class="mg-filter-body">
...
<div class="wrapper-field filter-buttons">
    <?php if ($data['submit']) { ?>
        ...
        <span class="refreshFilter"
              data-url="<?php echo SITE . URL::getClearUri() ?>"><?php echo lang('filterReset'); ?></span>
    <?php } else { ?>
        <a class="filter-now"><span><?php echo lang('filter'); ?></span></a>
        <a role="button" href="javascript:void(0);"
           class="refreshFilter"><span><?php echo lang('filterReset'); ?></span></a>
    <?php } ?>
</div>
...
<form name="filter" class="filter-form" action="<?php echo $arReuestUrl['path'] ?>"
