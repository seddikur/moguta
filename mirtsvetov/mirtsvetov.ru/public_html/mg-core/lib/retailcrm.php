...
class RetailCRM {
	...
	*/
	static function request($data, $url, $type){
		...
		if (!empty($response['failedOrders']) && !empty($response['errors'])) {
			$disableErrors = true;
			foreach ($response['errors'] as $responseError) {
				if (!preg_match('/^Order with externalId=[\d]+ already exists\.$/m', $responseError)) {
					$disableErrors = false;
					break;
				}
			}
			...
			if ($disableErrors) {
				...
				$response['uploadedOrders'] = $response['failedOrders'];
	...
	static function processUsers($result, $options){
		...
		foreach ($result['history'] as $key => $value) {
			...
			if ($value['customer']['site'] == $options['site']) {
				if (!isset($value['customer']['externalId'])) {
					continue;
				}
	...
	*/
	static function createUserArr($row, $options){
		...
		if (strlen($row['name']) > 100) {
			$row['name'] = mb_substr($row['name'], 0, 99, 'utf-8');
		}
		$data_customer = array(
			'externalId' => $row['id'],
			'firstName' => $row['name'],
