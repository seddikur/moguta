...
class DB {
  ...
  public static function init() {
    ...
    // Отключение кэширования статистики таблиц для mysql8+
    $mysqlVersion = self::getMysqlVersion();
    // Больше или равно 8.0.0
    // Меньше 9.0.0
    if (
      version_compare($mysqlVersion, '8.0.0') !== -1 &&
      version_compare($mysqlVersion, '9.0.0') === -1
    ) {
      DB::query('SET information_schema_stats_expiry = 0');
    }
  }
  ...
  public static function getMysqlVersion() {
    $mysqlVersion = mysqli_get_server_version(self::$connection);
