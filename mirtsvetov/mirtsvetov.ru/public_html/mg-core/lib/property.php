...
class Property {
	...
	public static function getEasyPropToCsv($id, $color, $size) {
		...
		if ((intval($color) + intval($size) === 0) || $neededProp === '0') {
			$neededProp = false;
		}
		if(!empty($neededProp))
	...
	static function addCategoryBinds($oldCatId, $newCatId, $propertys) {
	 ...
	 /**
     * Получает количество неиспользуемых значений характеристик
	 * <code>
	 * 	$res = Property::getCountUnusedPropertyValues();
	 *  viewData($res);
	 * </code> 
     * @return int
     */
	public static function getCountUnusedPropertyValues(){
		$getQuery = 'SELECT count(pd.id) AS count_props '.
					'FROM `'.PREFIX.'property_data` AS pd '.
					'LEFT JOIN `'.PREFIX.'property` AS p ON pd.prop_id = p.id '.
					'LEFT JOIN `'.PREFIX.'product_user_property_data` AS pupd ON pd.id = pupd.prop_data_id '.
					'WHERE p.type NOT IN ("size","color","assortmentCheckBox","assortment") AND pupd.prop_data_id IS NULL';
		$res = DB::query($getQuery);
		$countRows = DB::fetchAssoc($res);
		$result = intval($countRows['count_props']);
		return $result;
	}
	 ...
	 * <code>
	 * 	$res = Property::deleteUnusedPropertyValues();
	 *  viewData($res);
	 * </code> 
     * @return bool
     */
	public static function deleteUnusedPropertyValues(){
		$deleteQuery = 'DELETE pd '.
					   'FROM `'.PREFIX.'property_data` AS pd '.
					   'LEFT JOIN `'.PREFIX.'property` AS p ON pd.prop_id = p.id '.
					   'LEFT JOIN `'.PREFIX.'product_user_property_data` AS pupd ON pd.id = pupd.prop_data_id '.
					   'WHERE p.type NOT IN ("size","color","assortmentCheckBox","assortment") AND pupd.prop_data_id IS NULL';
