<?php
	...
	if(isset($_POST['login']) && $_POST['login'] == 'encodeupdate') {
		$config = get_ini_file('config.ini', true);
		$hostAndPort = explode(':', $config['DB']['HOST']);
		$port = null;
		$host = $config['DB']['HOST'];
	    ...
	    $connection = mysqli_connect($host, $config['DB']['USER'], $config['DB']['PASSWORD'], $config['DB']['NAME_BD'], $port);
	...
	if(isset($_POST['checkKey']) && $_POST['checkKey'] == 'encodeupdate') {
		$config = get_ini_file(REAL_DOCUMENT_ROOT.'/config.ini', true);
		$hostAndPort = explode(':', $config['DB']['HOST']);
		$port = null;
		$host = $config['DB']['HOST'];
		if(!empty($hostAndPort[1])){
		...
		if(!$keyInvalid) {
			...
			if((substr_count($res['msg'], 'не зарегистрирован для ключа') == 0)&&(substr_count($res['msg'], 'Ключ не найден!') == 0)) {
				if($res['status'] == 'error') {
					$msg = $res['msg'];
					if (strpos($msg, 'Ваша версия PHP не поддерживается!') === 0) {
						$msg = str_replace('Ваша версия PHP не поддерживается!', 'Ваша версия PHP '.PHP_MAJOR_VERSION.'.'.PHP_MINOR_VERSION.' не поддерживается!', $msg);
					}
				}
				$error = false;
			} else {
	...
	if(isset($_POST['update']) && $_POST['update'] == 'encodeupdate' || (isset($download) && $download)) {
		// считываем конфиг
		$config = get_ini_file(REAL_DOCUMENT_ROOT.'/config.ini', true);
		$key = '';
				...
				if($res['msg']) {
					...
					if(file_exists($file)) {
						...
						$mes = 'Файлы системы не подходят для вашей версии PHP. Была произведена попытка скачивания архива.<br>Архив не найден! Произошла ошибка при скачивании архива обновления. Вероятно ваша версия PHP не поддерживается.<br>
							Поддерживаемые версии PHP 5.6, 7.0, 7.1, 7.2, 7.3, 7.4, 8.1, 8.2, 8.3<br>У вас установлена версия PHP '.PHP_MAJOR_VERSION.'.'.PHP_MINOR_VERSION.'<br>
							Если ваша версия удовлетворяет требованиям, то попробуйте снова через 5 минут. В случае повторного возникновения этой ошибки через 5 минут - обратитесь в техподдержку Moguta.CMS';
	...
	$phpVersion = PHP_MAJOR_VERSION.'.'.PHP_MINOR_VERSION;
	if(!file_exists(REAL_DOCUMENT_ROOT.'/config.ini')) {
		file_put_contents(CORE_DIR.'lastPhpVersion.txt', $phpVersion);
	}
	...
	$timeDownload = null;
	if(is_file(CORE_DIR.'archiveDownloadTime.txt')){
	$timeDownload = @file_get_contents(CORE_DIR.'archiveDownloadTime.txt');
	}
	// проверка текущей версии и прошлой
	if((($lastPhpVersion != $phpVersion) && ($timeDownload < time())) || (isset($download) && $download)) {
