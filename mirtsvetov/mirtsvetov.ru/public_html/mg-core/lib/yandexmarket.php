...
//			}
			...
			if ($format === 'vk') {
				$nonEmptyDescriptionClauseField = 'description';
				if ($options['useShortDesc'] === 'true') {
					$nonEmptyDescriptionClauseField = 'short_description';
				}
				$nonEmptyDescriptionClause = '`'.$nonEmptyDescriptionClauseField.'` != "" AND `'.$nonEmptyDescriptionClauseField.'` IS NOT NULL';
				$filter .= ' AND '.$nonEmptyDescriptionClause.' ';
			}
			...
			while ($row = DB::fetchAssoc($res)) {
				...
				if (class_exists('SetGoods')) {
					...
					if ($complectComponentsRow = DB::fetchAssoc($complectComponentsResult)) {
						...
						if ($productsCodes) {
							...
							$setGoodsProducts = $catalogModel->getListByUserFilter(100, $setGoodsProductsWhereClause, true, true);
				...
				if ($printMain && $product['price'] == 0 && $options['productWithZeroPrice'] !== 'true') {continue;}
				...
				if ($printMain) {
					...
					if ($options['useOldPrice'] == "true"){
						$product['old_price'] = floatval($product['old_price'])/floatval($currencies[$options['uploadCurr']]);
				...
				if ($options['useVariants'] == 'true') {////////////////////////////////////////  start variants
					...
					foreach ($variants as $variant) {
						...
						if ($options['useOldPrice'] == 'true'){
							$variant['old_price'] = floatval($variant['old_price'])/floatval($currencies[$options['uploadCurr']]);
