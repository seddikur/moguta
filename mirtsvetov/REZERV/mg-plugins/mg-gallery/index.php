...
/*
  ...
  Edition: CLOUD
 */
  // 1.0.13 - убраны жесткие ссылки на домен
...
class mgGallery{
  ...
  static function preparePageSettings() {
    echo '   
      ...
      <script src="'.SITE.'/'.self::$path.'/js/admin.js"></script> 
    ';
  }
  ...
  static function handleGallery($args){
    ...
    while ($row = DB::fetchAssoc($res)) {
      $options = array(
        'in_line' => isset($options['in_line'])?$options['in_line']:$row['in_line'],
        'height' => isset($options['height'])?$options['height']:$row['height'],
        'id' => $options['id']
      );
    }

    $res = DB::query('SELECT id, id_gal, image_url, alt, title 
    FROM `'.PREFIX.'galleries_img` WHERE id_gal = '.DB::quote($options['id']).' ORDER BY abs(alt) ASC');
