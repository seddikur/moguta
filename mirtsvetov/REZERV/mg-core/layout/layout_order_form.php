		...
		switch ($field['type']) {
			...
			case 'select': ?>
				<li class="c-order__list--item c-order-field c-order__select js-orderFromItem flex100" <?php echo !empty($field['conditionType']) && $field['conditionType'] !== 'always'  ? $hide : ''; ?>>
					...
					<select name="<?php echo $fieldName ?>" title="<?php echo $field['placeholder']?$field['placeholder']:$field['title'] ?>" <?php echo $field['required'] ?>>
						<?php foreach ($field['vars'] as $option) { ?>
							<?php $selected = (isset($_POST[$fieldName]) && $_POST[$fieldName] == $option['val'] || $fieldName === 'shop_yur_id' && $option['val'] === 0) ? 'selected' : ''; ?>
			...
			case 'radiobutton': ?>
				<li class="c-order__list--item c-order__radiobutton c-order-field js-orderFromItem flex100" <?php echo !empty($field['conditionType']) && $field['conditionType'] !== 'always'  ? $hide : ''; ?>>
					...
					foreach ($field['vars'] as $option) { ?>
						<label <?php echo $active ?> title="<?php echo $field['placeholder'] ?>">
							<input name="<?php echo $fieldName ?>" value="<?php echo $option['val'] ?>" title="<?php echo $field['placeholder'] ?>"  
				...
				<?php break;
			case 'file': ?>
				<li class="c-order__list--item js-orderFromItem" <?php echo !empty($field['conditionType']) && $field['conditionType'] !== 'always'  ? $hide : ''; ?> style="white-space: nowrap;">
					<span><?php echo $field['title'] ?> :</span>
					<label class="orderFileContainer">
						<span class="orderFileText" data-initialtext="<?php echo lang('orderFormAddFile'); ?>"><?php echo lang('orderFormAddFile'); ?></span>
						<input style="display:none"	class="orderFileInput" type="file" name="<?php echo $fieldName ?>">
						<button class="removeOrderFile" style="display:none">×</button>
					</label>
				</li>
			<?php break;
		}
