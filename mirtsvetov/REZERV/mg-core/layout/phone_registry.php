<div class="js-phone-registry c-phone-registry c-form">
    ...
    <?php if($data['mode'] === 'replay'){ ?>
        <div class="alert alert-danger" role="alert">Неверный код.</div>
    <?php } ?>
    <p class="custom-text">Временный код успешно отправлен на номер телефона: <?php echo $data['userPhone']?></p>
    ...
    <div class="c-form__row">
        ...
        <?php if (!empty($data['capcha'])) { ?>
            <button type="submit" class="c-button c-button--link  default-btn" name="resendSMS" value="<?php echo $data['capcha']?>">Отправить повторно</button>
        <?php } elseif(!empty($_SESSION['timeSMS'])){ ?>
            ...
            <button type="submit" class="c-button c-button--link c-modal__cart default-btn" style="display: none;" name="timeSMS" value="<?php echo $_SESSION['timeSMS'];?>">Отправить повторно</button>
        <?php }?>
    </div>
</div>
