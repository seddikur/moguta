<?php if(($data['propertyOrder']['paymentAfterConfirm'] !== 'true' && !in_array($data['paymentId'], array(3, 4))) && (!isset($data['showPaymentForm']) || $data['showPaymentForm'])) { ?>
  <p class="confirm-info" style="font-size:12px;margin:0 0 10px 0; text-align: center; max-width: 675px">
    <a
      style="
      ...
      <td>
            ...
            <?php if (!empty($data['productPositions']) || $data['adminOrder']) : ?>
              ...
              <?php foreach ($data['productPositions'] as $product) : ?>
                ...
                <tr>
                  ...
                  <td style="font-size:13px;padding:5px 9px;" align="right"><?php echo MG::numberFormat($product['price'] * $product['count']).' '.$data['currency'] ?></td>
                  <?php $totalProductsSum += floatval($product['price']) * floatval($product['count']); ?>
                </tr>
              <?php endforeach; ?>
            <?php endif; ?>
          </tbody>

          <?php if (floatval($data['deliveryCost'])) : ?>
            <tr>
              <td colspan="3" align="right" style="padding:2px 9px 5px 9px; font-size: 13px;font-weight:bold;">
                <strong>Стоимость товаров:</strong>
              </td>
              <td align="right" style="padding:2px 9px 5px 9px; color: #333;text-align: center;font-size: 13px;font-weight:bold;">
                <strong><span><?php echo  MG::numberFormat($totalProductsSum).' '.$data['currency'] ?></span></strong>
              </td>
            </tr>
            <tr>
              <td colspan="3" align="right" style="padding:2px 9px 5px 9px; font-size: 13px;font-weight:bold;">
              ...
              <td align="right" style="padding:2px 9px 5px 9px; color: #333;text-align: center;font-size: 13px;font-weight:bold;">
                <strong><span><?php echo  MG::numberFormat($data['deliveryCost']).' '.$data['currency'] ?></span></strong>
              </td>
            </tr>
          <?php endif; ?>
          <tr>
        ...
        <table cellspacing="0" cellpadding="0" border="0" width="675" style="border:1px solid #EAEAEA;">
          ...
          <tbody>
            ...
            <tr><td style="font-size:13px;padding:5px 9px;"><b>Способ доставки</b>: <?php echo $data['delivery']; ?></td></tr>
            <?php
