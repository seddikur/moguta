$(document).ready(function() {
  ...
  if ($('.delivery-details-list input').length === 1 && 
      ...
      if(date_settings.trim() !== ''){ 
          ...
          $('.js-delivery-date input[name=date_delivery]').datepicker("option", "dateFormat", "dd.mm.yy");
          $('.js-delivery-date input[name=date_delivery]').datepicker("option", "minDate", date_settings.dateShift);
          if(typeof orderForm !== 'undefined'){
              $('.js-delivery-date input[name=date_delivery]').datepicker("option", "beforeShowDay", function(date) {
                  let day = date.getDay();
  ...
  $('body').on('change', '.delivery-details-list input', function() {
    ...
    if ($('.delivery-details-list li .active').data('delivery-use-storage') == '1') {
      $('.order-storage').show();
      $('.order-storage input').attr('required', true);
      $('.order-storage input').removeAttr('checked');
    } else {
      $('.order-storage').hide();
      $('.order-storage input').attr('required', false);
      $('.order-storage input').first().attr('checked', true);
    }
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        if ($('.payment-details-list input[name=payment]').length >= 1) {
          ...
          $('.payment-details-list input[name=payment]')[0].click();
        }
        if (!response.error) {
          if(response.summDelivery != 0) {
        ...
        const totalPriceString = document.querySelector('.total-sum:not(.order-summ)');
        if (!totalPriceString) return;
  ...
  $('.form-list select[name="customer"]').change(function() {
    if (typeof orderForm == 'undefined') {
      if ($(this).val() == 'fiz') {
        ...
        if ($('.payment-details-list input[name=payment]:visible').length >= 1) {
          $('.payment-details-list input[name=payment]:visible').first().trigger('click');
        }
      }
      if ($(this).val() == 'yur') {
  ...
  if ($('.payment-details-list input[name=payment]:visible').length >= 1) {
    $('.payment-details-list input[name=payment]:visible').first().trigger('click');
    $('.payment-details-list input[name=payment]:visible')[0].click();
  }
