	...
	return {
		...
		initEvents: function() {
			...
			$('#tab-shop-settings').on('click', '.viewAgreement', function() {
				...
				settings.layoutFromShopSettings = true;
				$('.section-settings .template-tabs-menu a[data-target="#ttab2"]').click();
				$('.section-settings .template-tabs-content #ttab2 a[data-filename="layout_agreement.php"]').click();
			});
			...
			$('#tab-shop-settings').on('click', '.show_catalog_sort_fields', function(){
				$('.catalog_sort_fields').toggle(200);
				$(this).hide();
			});

			$('#tab-shop-settings .catalog_sort_fields').sortable({
				handle: ".mover_catalog_sort_fields",
			...
			$('#tab-shop-settings').on('change',
			 ...
			 function() {
				...
				$('select[name="confirmRegistrationPhoneType"]').closest('.js-settline-toggle').hide();
				if (!$('#tconfirmRegistration').prop('checked')) {
					...
					if (!$('#tconfirmRegistrationPhone').prop('checked') && !$('#tconfirmRegistrationEmail').prop('checked')) {
						$('#tconfirmRegistrationPhone').prop('checked', true);
						$('#tconfirmRegistrationEmail').prop('checked', true);
					}
					if ($('#tconfirmRegistrationPhone').prop('checked')) {
						$('select[name="confirmRegistrationPhoneType"]').closest('.js-settline-toggle').show();
					}
				}
          ...
          $('input[name="ownerRememberEmail"]').closest('.js-settline-toggle').hide();
			...
			// Отдельная скрывалка для типа подтверждения по телефону
			$('#tab-shop-settings').on('change', '#tconfirmRegistrationPhone', function() {
				if ($('#tconfirmRegistrationPhone').prop('checked')) {
					$('select[name="confirmRegistrationPhoneType"]').closest('.js-settline-toggle').show();
				} else {
					$('select[name="confirmRegistrationPhoneType"]').closest('.js-settline-toggle').hide();
