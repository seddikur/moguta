 ...
 */
 var order = (function() {
    return {
            ...
            select.bind('change', function() {
                ...
                if (typeof data === 'object' && data !== null) {
                    Object.keys(data).forEach(function(key) {
                        var clearVal = typeof data[key] === "string" ? admin.htmlspecialchars_decode(data[key]) : '';
                        $('.section-order #additional-requisites-modal input[name=' + key + ']').val(clearVal);
                    });
                }
              ...
              // для выбора юр лиц в настройках заказа
              var selectOptionReq = $('form[name="requisites"] select[name="tempRequisites"]');
              var previousValueSel = selectOptionReq.val();
              selectOptionReq.bind('change', function() {

                  var currentValue = selectOptionReq.val();
                  var data = admin.createObject('.optionAddRequisitesInput');
  
                  if (previousValueSel !== null) {
                    selectOptionReq.find('option[value=' + previousValueSel + ']').data('data', data).text(data.nameadmin);
                  }
  
                  data = admin.tryJsonParse(selectOptionReq.find('option[value=' + currentValue + ']').data('data'));
                  if (typeof data === 'object' && data !== null) {
                      Object.keys(data).forEach(function(key) {
                         var clearVal = typeof data[key] === "string" ? admin.htmlspecialchars_decode(data[key]) : '';
                         var inpt = $('form[name="requisites"] input[name=' + key + ']').val(clearVal);
                         var inptVal = inpt.val();
                         if(key == "stamp" || key == "sing") { 
                            (inptVal) ? inpt.siblings('img').attr('src',  mgBaseDir + '/' + inptVal) : inpt.siblings('img').attr('src', mgBaseDir + '/uploads/' + key + '.jpg'); 
                        }
                         if(inptVal == 'true' || inptVal === true) inpt.prop('checked', true);
                         if(inptVal == 'false' || inptVal === false) inpt.prop('checked', false);                         
                      });
                  }
  
                  previousValueSel = currentValue;
              });
        ...
        initReqChoose: function(){           
            admin.ajaxRequest(
                {
                    mguniqueurl: 'action/loadAddRequisites',
                },
                function(response) {
                    var select = $('form[name="requisites"] select[name="tempRequisites"]');
                    if (response.data) {
                        Object.keys(response.data).forEach(function(key) {
                            var tmp = admin.tryJsonParse(response.data[key]);
                            select.append('<option value="' + key + '"' + (tmp.choosen === 'true' ? "selected" : "") + '>' + tmp.nameadmin + '</option>');
        ...
        initEvents: function() {
            ...
            // ================================
            $('.section-order').on('click', '.openAddRequisitesModal', function(event) {
                ...
                admin.ajaxRequest({
                    ...
                    function(response) {                        
                        ...
                        if(reqKeys.length){
                            order.lastMaxId = Math.max(...reqKeys, order.lastMaxId);
                        }
            ...
            $('.section-order').on('click', '#additional-requisites-modal .add-requisites', function() {
                var select = $('.section-order #additional-requisites-modal .select-requisites:first');
                var num = order.lastMaxId = parseInt(order.lastMaxId) + 1;
            ...
            $('.section-order').on('click', '#additional-requisites-modal .save-button', function() {
                ...
                $('.section-order #additional-requisites-modal .select-requisites option').each(function() {
                    var tmp = admin.tryJsonParse($(this).data('data'));
                    var num = $(this).attr('value');
                    if(!num.match(/^\d+$/)){
                      num = order.lastMaxId = parseInt(order.lastMaxId) + 1;
                    }                     
                    data[num] = tmp;
                });          
       ...
       createPositionPopup:function(position){
        ...
        saveOrder: function(id, container, number, closeModal) {
           ...
           var orderContent = order.orderItems;
            ...
            var packedProperty = {
                ...
                shop_yur_id: (yur ? container.find('.yur-list-editor select[name=shopYurId]').val() : ''),
              ...
              order.storage_count_array = [];
                ...
                if (response.data.order.pay_hash) {
                    ...
                    if (payHash.length < 32) {
                        ...
                        $('#add-order-wrapper #payLinkText').val(payLink);
                    ...
                    select: function(event, ui) {
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                $('#add-order-wrapper .editor-block input[name=fio_sname]').val(user.sname);
                                $('#add-order-wrapper .editor-block input[name=fio_name]').val(user.name);
                                $('#add-order-wrapper .editor-block input[name=fio_pname]').val(user.pname);
        ...
        drawOrder: function(response, data) {
            ...
            var shopYurInfo = '';      
            $.each(response.data.order.shop_yur_id, function(i, element) {
                shopYurInfo += "<option value=" + (i != 0 ? i : 0) + " " + (element.select ? 'selected' : '') + ">" + (element.nameyur || element.nameadmin || lang.REQUISITES_DEFAULT) + "</option>"
            });
            var weight = 0;
            $.each(response.data.order.order_content, function(i, element) {
              ...
              orderPersonalHtml += '\
                  <div class="row sett-line">\
                      <div class="row">\
                          ...
                          <div class="large-6 small-12 columns"><strong ' + tooltipForDisabled + '"><input readonly type="text" value="' + admin.htmlspecialchars(admin.htmlspecialchars_decode(response.data.order.name_buyer)) + '"></strong></div>\
                      </div>\
                  </div>\
                  <div class="row sett-line">\
                      <div class="row"> \
      ...
      ';
          ...
          <div class="order-fieldset order-payment-sum__item order-other-info order-edit-display editor-block shadow-block">\
            ...
            // Вёрстка данных юрлица
                   ...
                   <div class="order-fieldset order-payment-sum__item order-edit-visible order-other-info editor-block shadow-block">\
                        <h2 class="order-fieldset__h2">Реквизиты покупателя</h2>\
                        <div class="order-fieldset__inner">\
                            <div class="row sett-line">\
                                <div class="row">\
                        ...
                        <h2 class="order-fieldset__h2" style="padding-top:0px;">Реквизиты продавца</h2>\
                        <div class="row sett-line">\
                            <div class="row" style="padding: 10px 20px;">\
                                ...
                                <div class="large-7 small-12 columns">\
                                    <select disabled id="shopYurId" name="shopYurId">' + shopYurInfo + '</select>\
                                </div>\
                            </div>\
                        </div>\
                   </div>\
          ...
          <div class="order-fieldset order-payment-sum__item order-edit-display order-other-info js-yur-list-toggle editor-block shadow-block">\
            ...
            <h2 class="order-fieldset__h2" style="padding-top:0px;">Реквизиты продавца</h2>\
            <div class="row sett-line yur-list-editor">\
                <div class="row" style="padding: 10px 20px;">\
                    <div class="large-6 small-12 columns">\
                        <label>' + lang.OREDER_LOCALE_9 + ':' + '</label>\
                    </div>\
                    <div class="large-6 small-12 columns">\
                        <select id="shopYurId" name="shopYurId">' + shopYurInfo + '</select>\
                    </div>\
                </div>\
            </div>\
        </div>';
      ...
      ';
                        ...
                        <div class="small-12 medium-6 columns ">\
                            <strong>\
                                <div class="payLinkContainer">\
                                    <input id="payLinkText" type="text" readonly />\
       ...
       <li class="delete-order" id="' + element.id + '"><a class="tool-tip-bottom fa fa-trash" href="javascript:void(0);" title="' + lang.DELETE + '"></a>\
        ...
        enableEditor: function() {
            ...
            const nameBuyerInput = $('.js-name-toggle input[name="name_buyer"]');
            nameBuyerInput.val(admin.htmlspecialchars_decode(nameBuyerInput.val()));
        ...
        addToOrder: function(obj, data=false) {
            ...
            $('.status-table tbody#orderContent tr').each(function(i, element) {
                var thisClassNameAttribue = $(this).attr('class');
                if (thisClassNameAttribue.indexOf('emptyOrder') >= 0) {
                    return;
                }
                if (!$(this).data('id')) {
                    return true;
                ...
                var variant1 = parseFloat(position.variant);
