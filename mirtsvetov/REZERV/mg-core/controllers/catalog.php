...
class Controllers_Catalog extends BaseController {
  ...
  function __construct() {
    ...
    if ($keyword) {
      ...
      if ($data['cat_id'] > 0) {
        $data['cat_opFields'] = Models_OpFieldsCategory::getContent($data['cat_id'], false);
      }
    }
  ...
  public function getSearchData() {
    ...
    if (!empty($keyword)) {
      ...
      foreach($items['catalogItems'] as &$product){
        if(!empty($product['variant_id'])){
          $sql = 'SELECT product_id, variant_id, price FROM '.PREFIX.'wholesales_sys 
            WHERE product_id = '.DB::quoteIN($product['id']).' AND variant_id = '.DB::quoteIN($product['variant_id']).' AND `count` = 1 AND `group` = '.User::access('wholesales');
        }else{
          $sql = 'SELECT product_id, variant_id, price FROM '.PREFIX.'wholesales_sys 
            WHERE product_id = '.DB::quoteIN($product['id']).' AND variant_id = 0 AND `count` = 1 AND `group` = '.User::access('wholesales');
        }
        if($res = DB::query($sql)){
