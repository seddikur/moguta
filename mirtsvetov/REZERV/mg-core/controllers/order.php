...
class Controllers_Order extends BaseController {
  ...
  public function getDelivery() {
    ...
    $model = new Models_Order;
    $cart = new Models_Cart;
    $cartSumm = $cart->getTotalSumm(true);
  ...
  private function redirectUserToAuthOrLk() {
    $orderOnlyForConfirmedUsers = MG::getSetting('orderOnlyForConfirmedUsers');
    if ($orderOnlyForConfirmedUsers !== 'true') {
      return false;
    }

    $currentUser = USER::getThis();
    if (empty($currentUser)) {
      MG::redirect('/enter', '303 See Other');
      return true;
    }
    if (!empty($currentUser->activity) && intval($currentUser->activity) === 1 && intval($currentUser->blocked) === 0) {
      return false;
    }

    MG::redirect('/personal', '303 See Other');
    return true;
  }
