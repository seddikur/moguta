...
class Controllers_Api extends BaseController {
	...
	public static function importCategory($param = null) {
		foreach ($param['categories'] as $category) {
			...
			if(empty($category['id'])) {
				$categoryLib->addCategory($category);
			} else {
				$categoryLib->unsetListCategoryId();
				$categoryLib->updateCategory($category);
			}
		}
		return 'Импорт завершен';
	}
	...
	public static function getProduct($param = null) {
		...
		if($param['property']) {
			...
			$res = DB::query('SELECT pupd.id, pupd.product_id, pupd.prop_id, pupd.prop_data_id, IF(pupd.name = \'\', pd.name, pupd.name) AS name, pupd.margin 
				...
				WHERE pupd.prop_id IN ('.DB::quoteIN($propertiesIds).') 
				AND pupd.product_id IN ('.DB::quoteIN($productsIds).')');
			while($row = DB::fetchAssoc($res)) {
				$propertyData[] = $row;
			}
					...
					if($data['product_id'] == $value['id']) {
						foreach ($value['property'] as $keyP => $valueP) {
							if($valueP['id'] == $data['prop_id']) {
								...
								if (!empty($result['products'][$key]['property'][$keyP]['data'])) {
	 ...
	 * 	'description' : => '<p>Описание товара</p>',	// описание товара
	 * 	'short_description' : => '<p>Краткое описание товара</p>',	// краткое описание товара
	 * 	'price' : => 87894,	// цена
	 * 	'url' : => 'raspredelitelnyy-elektroshkaf',	// последняя секция урла
	 * 	'image_url' : => 'no-img.jpg',	// ссылки на изображения, разделитель | . Можно указать название файла с изображением, если он уже располагается в директории товара или указать ссылку на изображение и она будет загружена к товару (при загрузке иозбражений по ссылке, метод запроса к API должен быть POST)
	 ...
	 *  'opf_1' : => 'Значение первого дополнительного поля', // дополнительные поля товара (если есть)
	 *  'opf_2' : => 'Значение второго дополнительного поля', // дополнительные поля товара (если есть)
	...
	public static function importProduct($param = null) {
		...
		foreach ($param['products'] as $item) {
			...
			if (isset($item['image_url'])) {
				$imagesString = $item['image_url'];
				$images = explode('|', $imagesString);
				$preparedImages = [];
				foreach ($images as $image) {
					if (strpos($image, 'http') === 0) {
						$urlImagePathes = self::getUrlImagePaths($image);
						if ($urlImagePathes) {
							$preparedImages[] = $urlImagePathes['pathToDB'];
							$imagesForMoving[] = $urlImagePathes['pathToMove'];
						}
					} else {
						$preparedImages[] = $image;
					}
				}
				$item['image_url'] = implode('|', $preparedImages);
			}
			...
			if(empty($item['id'])) {
				...
				// Перемещаем изобаржения, загруженные по ссылке
				if ($imagesForMoving) {
					$model->movingProductImage($imagesForMoving, $prodId, 'uploads/prodtmpimg');
				}
			} else {
				...
				if($res = DB::fetchAssoc($res)) {
					...
					// Перемещаем изобаржения, загруженные по ссылке
					if ($imagesForMoving) {
						$model->movingProductImage($imagesForMoving, $item['id'], 'uploads/prodtmpimg');
					}
				} else {
	...
	private static function getUrlImagePaths($imageUrl) {
