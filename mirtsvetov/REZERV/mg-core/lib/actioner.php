...
class Actioner {
	...
	public function saveProduct() {
		...
		if (!empty($_POST['id'])) {
			...
			// костыль
			$arrVar = $model->getVariants($_POST['id'], false, false, false, true);
			$minVarPrice = array();
		...
		if (isset($_POST['variants']) && is_array($_POST['variants'])) {
			...
			foreach ($_POST['variants'] as $item) {
				if(!empty($item['color'])){
					$propValues[$item['color']] = $propIdColor;
				}
				if(!empty($item['size'])){
					$propValues[$item['size']] = $propIdSize;
				}	
			}
			if(!empty($propValues) && MG::get('disableVariantSizeMapSave') !== 1){
				$arrParts = [];
				foreach($propValues as $idPropVal => $idProp){
					$arrParts[] = '('.DB::quoteInt($idProp).', '.DB::quoteInt($idPropVal).', '.DB::quoteInt($_POST['id']).', 1)';
				}
				$insertPropValues = 'INSERT INTO '.PREFIX.'product_user_property_data '.
				'(prop_id, prop_data_id, product_id, active) '.
				'VALUES '.implode(',', $arrParts).'';
				DB::query($insertPropValues);
			}	
		}
	...
	public function saveOrder() {
        ...
        foreach ($_POST['order_content'] as &$value){
          if(isset($value['storage_id'])){
            ...
            $value['storage_id'] = array_filter($storage_id);
          }
        }    
		...
		if(!empty($_POST['shop_yur_id'])){
			$_POST['shop_yur_id'] = htmlspecialchars($_POST['shop_yur_id']);
		} else {
			$_POST['shop_yur_id'] = "";
		}
	...
	public function editSettings() {
		...
		if (!empty($_POST['options'])) {
			...
			// UPD
			// Теперь подтверждение можно включить и для телефона и для почты одновременно
			// Но если ни одни из способ не включен, то опция подтверждения отключается
			...
			if ($confirmRegistration) {
				if (!$confirmRegistrationEmail && !$confirmRegistrationPhone) {
					$confirmRegistration = false;
					$_POST['options']['confirmRegistration'] = 'false';
				}
			}
			////
			////
			...
			foreach ($_POST['options'] as $option => $value) {
				...
				if($option === 'sortFieldsCatalog'){
					$inComingSortFields = json_decode($_POST['options']['sortFieldsCatalog'], true);
					$sortFields = MG::getSetting('sortFieldsCatalog', true);
					foreach($inComingSortFields as $nameField => $enableValue){
						$sortFields[$nameField]['enable'] = $enableValue; 
						$inComingSortFields[$nameField] = $sortFields[$nameField];			
					}					
					$value = addslashes(serialize($inComingSortFields));
				}
	...
	public function saveDeliveryMethod() {
		...
		$dateSettings = [
			...
			'daysWeek' => $daysWeek,
			'monthWeek' => $monthWeek,
			'requiredDate' => (int)$_POST['requiredDate']
		];
		...
		if (intval($showStorages)) {
			$storages = MG::getSetting('storages', true);
			$noPickupStorage = true;
			foreach ($storages as $storage) {
				if ($storage['pickupPoint'] === 'true') {
					$noPickupStorage = false;
					break;
				}
			}
			if ($noPickupStorage) {
				$this->messageError = $this->lang['DELIVERY_STORAGES_ERROR'];
				return false;
			}
		}
	...
	*/
	...
	public function operationProduct() {
		...
		if ($operation == 'delete') {
			...
			foreach ($_POST['products_id'] as $product) {
				...
				if (!empty($_POST['data']['csv_encoding'])) {
					$_POST['encoding'] = $_POST['data']['csv_encoding'];
				}
	...
	public function operationCategory() {
		...
		if ($operation == 'delete') {
			...
			Storage::clear();
		} elseif (strpos($operation, 'changeProductCategory') === 0){
			if(empty($_POST['param']) || empty($_POST['category_id'])) { 
				$this->messageError = $this->lang['CHECK_SELECTED_CATEGORY'];
				return false;
			} 
			$categoryIds = $_POST['category_id'];
			if(!empty($_POST['nestedCategory'])){
					$category = new Category;
					foreach($_POST['category_id'] as $categoryId){
						if(!$cat = $category->getChildsCategory($categoryId)){ continue; }
						$categoryIds = array_merge($categoryIds, $cat);
					};
				}
				$catId = implode(',', array_unique($categoryIds)); 			
				$productModel = new Models_Product;
				$res = DB::query('SELECT `id` FROM `'.PREFIX.'product` WHERE `cat_id` IN ('.$catId.')');
				while($row = DB::fetchAssoc($res)) {
					$productModel->updateProduct(array('id' => $row['id'], 'cat_id' => $_POST['param']));
				}
		}
 ...
 /**
	...
	public function getOrderData() {
      ...
      foreach ($orderData['order_content'] as &$items) {                                
		...
		$shopReqs[] = MG::getSetting('propertyOrder', true);
		$addReqs = MG::getSetting('addRequisites',true);
		if(!empty($addReqs)){
			foreach($addReqs as $key => $shopReq){
				$shopReqs[$key] = $shopReq;
				if($key === intVal($response['order']['shop_yur_id'])){
					$shopReqs[$key]['select'] = true;
				}
			}	
		}
		
		$response['order']['shop_yur_id'] = $shopReqs;
		...
		if(!empty($_POST['tempRequisites'])){
			...
			if(!empty($addReqs)){
				...
				foreach($addReqs as $key => $req){
					$addReqs[$key]['choosen'] = 'false';
				}
				if(!empty($addReqs[$_POST['tempRequisites']])){
					$idReq = $_POST['tempRequisites'];
					unset($_POST['tempRequisites']);
					foreach($_POST as $key => $req){
						if(in_array($key, $exceptionFields)) { 
							$propertyOrder[$key] = $req;
							continue;
						 }
						$addReqs[$idReq][$key] = $req;
					}
					$addReqs[$idReq]['choosen'] = 'true';
				}
				MG::setOption('addRequisites', $addReqs, true);	
			}
			...
			MG::setOption('propertyOrder', $propertyOrder, true);
		} else {
			$this->messageError = $this->lang['SAVE_FAIL'];
			return false;
		}
...
<td class='actions'>
  ...
  public function convertImgToWebp() {
	...
	*/
	...
	public function copyTemplateFile() {
		...
		if (
			(
				substr($_POST['file'], 0, 6) == 'email_' ||
				$_POST['file'] === 'layout_agreement.php'
			) &&
			empty($_POST['userAction']) &&
			$_POST['file'] != 'email_template.php' &&
			$_POST['file'] != 'email_order_admin.php'
		) {
			return true;
		}
   ...
   */
	...
	public function getUserOrderContent() {
		...
		$userEmail = '';
		$res = DB::query('SELECT `login_email` as email FROM '.PREFIX.'user WHERE id = '.DB::quoteInt($_POST['id']));
		if($row = DB::fetchAssoc($res)) {
			$userEmail = $row['email'];
		}
	...
	function saveAddRequisites() {
		...
		$flagUpdOrder = false;
		if(!empty($_POST['data'] && !empty($addReqs))){
			if(count($_POST['data']) < count($addReqs)){
				$flagUpdOrder = true;
			}
		}

		if(empty($_POST) && !empty($addReqs) || $flagUpdOrder){
			$delReqsIds = [];
			$updOrdersIds = [];
			foreach($addReqs as $key => $req){
				if(!isset($_POST['data'][$key])){
					array_push($delReqsIds, $key);
				}
			}
			$delReqsIds = implode(',', $delReqsIds);
			$orderQueryId = "SELECT `id` FROM `".PREFIX."order` WHERE `shop_yur_id` IN (".DB::quote($delReqsIds, 1).")";
			$res = DB::query($orderQueryId);
			while($orderId = DB::fetchRow($res)){
				array_push($updOrdersIds, $orderId[0]); 
			}
			if($updOrdersIds){
				$updOrdersIds = implode(',', $updOrdersIds);
				$updOrder = "UPDATE `".PREFIX."order` SET `shop_yur_id` = '' WHERE `id` IN (".$updOrdersIds.")";
				DB::query($updOrder);
			}
		} else {

			if(!empty($_POST['data'])){
				foreach($_POST['data'] as $key => $rqVal){
					$_POST['data'][$key]['stamp'] = !empty($addReqs[$key]['stamp']) ? $addReqs[$key]['stamp'] : 'uploads/stamp.jpg';
					$_POST['data'][$key]['sing'] = !empty($addReqs[$key]['sing']) ? $addReqs[$key]['sing'] : 'uploads/sing.jpg';
					$_POST['data'][$key]['usedsing'] = !empty($addReqs[$key]['usedsing']) ? $addReqs[$key]['usedsing'] : "true";
				}	
			}
		}			 
	...
	*/
	...
	public function applyTemplateSettings() {
    	...
    	if (!empty($templateSettings)) {
          ...
          'printSpecFilterBlock', 'printPriceFilterBlock', 'printFilterResult', 'filterCountProp', 'filterMode', 'filterCountShow', 'filterSubcategory', 'searchType', 'searchInDefaultLang',
          'disabledPropFilter', 'filterCatalogMain', 'useSearchEngineInfo', 'productFilterPriceSliderStep', // фильтры и поиск по сайту
          'popupCart', 'connectZoom', 'printCompareButton', 'compareCategory', 'copyrightMoguta', 'copyrightMogutaLink', 'sortFieldsCatalog', // опции сайта (общие)
          ...
          'varHashProduct', 'catalogProp', 'outputMargin', 'sizeMapMod', 'printOneColor', 'recalcWholesale', 'recalcForeignCurrencyOldPrice', 'convertCountToHR','printTypeProduct', // опции сайта (товары)
          'orderNumber', 'prefixOrder', 'orderOwners', 'usePhoneMask', 'phoneMask', 'deliveryZero', 'enableDeliveryCur','enableDeliveryCostDiscount', // опции сайта (заказы)
  ...
  public function importCatalogYML() {
    ...
    $newFileStructure = (!empty($arParams['new_file_structure']) &&
      ...
      true : false;
	$importInWarehouse = intval($arParams['import_in_warehouse']);

	if($arParams['changeRest'] !== ''){
		$changeRest = ($arParams['changeRest'] >= -1) ? intval($arParams['changeRest']) : -1;	
	} else {
		$changeRest = '';
	}
    ...
    switch ($step) {
      ...
      case 3:
		...
		$doNotUpdateExistingProductsCats = false;
		if ($arParams['doNotUpdateExistingProductsCats'] === 'true') {
			$doNotUpdateExistingProductsCats = true;
		}
        ...
        $resultData = ymlImport::parseProductsToDb($file, $priceModifier, $onlyPrice, $importCategory, $importInCategory, $newFileStructure, $onlyNewProductsImages, $importInWarehouse, $doNotUpdateExistingProductsCats, $changeRest);
        break;
      case 4:
  ...
  public function disableDowntime() {
	if (USER::access('setting') < 2) {
		$this->messageError = $this->lang['ACCESS_EDIT'];
		return false;
	}
	MG::setOption('downtime', 'false');
	MG::setSetting('downtime', 'false');
	return true;
  }
}
