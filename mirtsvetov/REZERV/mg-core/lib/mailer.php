...
class Mailer {
  ...
  public static function sendMimeMail($dataMail) {
    ...
    // Переводим email'ы, содержащие кириллицу в punnycode
    $toPunnyFields = [
        'emailFrom',
        'emailTo',
    ];

    require_once(CORE_DIR.'lib/idna_convert.class.php');
    $idn = new idna_convert(array('idn_version' => 2008));
    foreach ($dataMail as $name => $value) {
        if (in_array($name, $toPunnyFields)) {
            $encodedValue = $idn->encode($value);
            if (stripos($encodedValue, 'xn--') !== false) {
                $dataMail[$name] = $encodedValue;
            }
        }
    }
    ...
    if (!empty($dataMail['addheaders'])) {
        $m->addHeaders($dataMail['addheaders']);
    }
    ...
    if(MG::getSetting('smtp')==="true"){
      ...
      if (!$conAr['host'] || !$conAr['login'] || !$conAr['pas']) {
        return false;
      }
    ...
    if (isset($dataMail['attach']) && is_array($dataMail['attach'])) {
    ...
    LoggerAction::logAction('Email', __FUNCTION__, array(
        'to' => $dataMail['emailTo'],
   ...
   *
   * @param string $host хост SMTP
   * @param string $login логин
   * @param string $pas пароль
   * @param string $port порт подключения
   * @param string $timeout таймаут ожидания
  ...
  public static function setSMTPDataConnect($host, $login, $pas, $port, $timeout, $emailFrom, $clientEmail = '') {
    ...
    // Переводим мыло в punnycode, если оно содержит кириллицу
    $toPunnyFields = [
        'login'
    ];
  ...
  public static function sendMimeMailWithFile($dataMail) {
    return self::sendMimeMail(array(	  
          ...
          'body' => $dataMail['text'],
          'html' => false,
          'addheaders' => !empty($dataMail['addheaders']) ? $dataMail['addheaders'] : [],
          'attach' => array(
            array(
...
class Mail
{
    ...
    public function Send()
    {
        ...
        if (!$this->smtp['on'])
        {
            ...
            foreach ($this->sendto as $key => $value)
            {
                ...
                {   // если текущий ресурс не является ресурсом по умолчанию, удалим заголовки этого ресурса, так как они уже не нужны
                    // а вот заголовки от ресурса по умолчанию еще могут понадобится
                    unset($this->headers[$key]);
                    unset($this->ready_headers[$key]);
                }
                // если ресурс для тела не является ресурсом по умолчанию, удалим тело этого ресурса, оно уже не понадобится
                // а вот тело ресурса по умолчанию еще может понадобится
        ...
        else // если через smtp
        {
          ...
          if (defined("NEW_SMTP_TLS") && NEW_SMTP_TLS == 1 && $this->smtp['port'] == 587) {
            $socket_context = stream_context_create([]);
              $errno = 0;
              $errstr = '';
              $smtp_conn = stream_socket_client(
                MG::getSetting('smtpHost') . ':' . $this->smtp['port'],
                $errno,
                $errstr,
                $this->smtp['timeout'],
          ...
          if (defined("NEW_SMTP_TLS") && NEW_SMTP_TLS == 1 && $this->smtp['port'] == 587) {
              ...
              stream_socket_enable_crypto($smtp_conn, true, $crypto_method);
            ...
            foreach ($this->smtpsendto as $key_res => $value_res)
            {
                ...
                // если ресурс не является ресурсом по умолчанию, удалим заголовки этого ресурса, они уже не понадобятся, а вот заголовки ресурса по умолчанию могут понадобится                
                if ($key_res != 'webi')
                {
                    unset($this->headers[$key_res]);
                    unset($this->ready_headers[$key_res]);
                }
                // если ресурс для тела не является ресурсом по умолчанию, удалим тело этого ресурса, оно уже не нужно, а вот тело ресурса по умолчанию еще может понадобится
                if ($body_resource != 'webi')
                {
                    unset($this->body[$body_resource]);
                }
