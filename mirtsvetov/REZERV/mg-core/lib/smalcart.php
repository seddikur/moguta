    ...
    if (!empty($_SESSION['cart'])) {
      ...
      foreach ($_SESSION['cart'] as $key => $item) {
        ...
        if (!empty($item['variantId']) && !empty($array_variants[$item['variantId']])) {
          ...
          foreach($variant as $variantFieldName => $variantFieldValue){
            if(strpos($variantFieldName, 'opf_') !== false){
              if(!empty($row[$variantFieldName])){
                $row[$variantFieldName] = $variantFieldValue;
              }
            }
          }
        ...
        if ($item['id'] == $row['id']) {
          ...
          if (floatval($row['old_price']) > floatval($price)) {
            $row['old_price'] = self::plusPropertyMargin($row['old_price'], $item['propertyReal'], $currencyRate[$row['currency_iso']]);
          }
