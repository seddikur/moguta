...
class Models_Cart {
  ...
  public function createProperty($arr) {
    ...
    foreach ($arr as $key => $value) {
      ...
      if (!empty($parseData)) {
        ...
        // для отрезания метки у названия характеристики
        $tmp = explode('[prop', $realName);
        $realName = $tmp[0];
        
        if (empty($data) && !empty($realVal)) {
          $data['name'] = str_replace('%', '', $realVal);
          $data['margin'] = 0;
        }
        if (!empty($data)) {
          $plus = $product->addMarginToProp($data['margin'], $currencyRate );
          $plus = MG::getSetting('outputMargin') === 'true' ? $plus : '';
        ...
        if (is_array($item['thisUserFields'])) {
  ...
  public function delFromCart($id, $property, $variantId) {
    ...
    $args = func_get_args();
    MG::createHook(__CLASS__."_".__FUNCTION__, null, $args);
  }
   ...
   * </code>
   * @param $costWithoutAnyDiscountFlag - при значении true учитывает опцию из админ панели для расчета бесплатной доставки, если опция включена возвращает цену без скидок и купонов
   * @return float
   */
  public function getTotalSumm($costWithoutAnyDiscountFlag = false) {
    ...
    $isCostWithoutAnyDiscount = MG::getSetting('enableDeliveryCostDiscount') === 'true' && $costWithoutAnyDiscountFlag;
    ...
    if (!empty($_SESSION['cart'])) {
      ...
      foreach ($_SESSION['cart'] as $key => $item) {
        ...
        $prod['price'] = MG::convertPrice($prod['price']);
        $wholesalePrice = MG::setWholePrice($prod['price'], $item['id'], $item['count'], $item['variantId'], $currency_iso);
       // $prod['price'] = min([$prod['price'], $wholesalePrice]);
        $prod['price'] = $wholesalePrice;
        ...
        if (floatval($prod['old_price']) > floatval($prod['price'])) {
          $prod['old_price'] = SmalCart::plusPropertyMargin($prod['old_price'], $item['propertyReal'], $currencyRate[$prod['currency_iso']]);
        }
    ...
    if (!empty($productPositions)) {
      if(!($isCostWithoutAnyDiscount)){
        ...
        foreach ($productPositions as $key => $product) {
          // считаем обычную цену   
          $product['price'] = MG::roundPriceBySettings($product['price']);          
          $totalSumm += $_SESSION['cart'][$product['keyInCart']]['count'] * $product['price'];
        }
      }
    }
      ...
      foreach ($_SESSION['cart'] as $key => $item) {
        ...
        if (!empty($product)) {
          ...
          if (!empty($variant)) {
            ...
            foreach($variant as $variantFieldName => $variantFieldValue){
              if(strpos($variantFieldName, 'opf_') !== false){
                if(!empty($product[$variantFieldName])){
                  $product[$variantFieldName] = $variantFieldValue;
                }
              }
            }
          }
          ...
          if ($item['id'] == $product['id']) {
            $count = $item['count'];
	         ...
	         $currency_iso = 'RUR';
            // сюда вставляем цену оптовую, которая с админки
            $resIn = DB::query('SELECT currency_iso FROM '.PREFIX.'product WHERE id = '.DB::quoteInt($item['id']));
            if($rowIn = DB::fetchAssoc($resIn)) {
              $currency_iso = $rowIn['currency_iso'];
            }
            ...
            $wholesalePrice = MG::setWholePrice($price, $product['id'], $item['count'], isset($product['variantId'])?$product['variantId']:null, $currency_iso);
            //$price = min([$price, $wholesalePrice]);
            $price = $wholesalePrice;
            
            if (floatval($product['old_price']) > floatval($price)) {
              $product['old_price'] = SmalCart::plusPropertyMargin($product['old_price'], $item['propertyReal'], $currencyRate[$product['currency_iso']]);
            }
            ...
            if(MG::enabledStorage()) {
              ...
              if ($storage && $storage !== 'all') {
                $product['count'] = $productModel->getProductStorageCount($storage, $product['id'], 0);
              } else {
                $product['count'] = $productModel->getProductStorageTotalCount($product['id'], 0);
              }
            } 
